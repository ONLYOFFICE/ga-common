...
- The Docker sandbox is the CLI's containment now, not a tool allowlist: the review container has only `ANTHROPIC_API_KEY` (no `GITEA_TOKEN`/`BUGZILLA_API_KEY`, no `docker.sock`), sits on an `--internal` network with no direct route out, and can only reach the npm registry and the Anthropic API through a Squid allowlist proxy — that containment is *why* `--allowedTools` was dropped entirely in favor of full access (`--dangerously-skip-permissions`, run as the image's unprivileged `node` user, never root — the CLI itself refuses `--dangerously-skip-permissions` as root). The CLI still runs against `/workspace` = the PR's own checkout copied in via `docker cp`, so the reviewed repo's own `.claude/` settings, hooks and skills are still in scope of its normal auto-loading — the sandbox limits what a compromised session can *reach*, it doesn't stop the session from being influenced by the repo's own config in the first place. `/output` is the one exception to "nothing is bind-mounted" — a dedicated empty host directory (not the job's real workspace) for `--debug-file`/the result JSON only, so a compromised session can write its own output there and nothing else on the host. Keep the egress allowlist tight, and never mount `docker.sock`, the job's real workspace, or real secrets into the review container itself. The sandbox and proxy are sibling containers on the runner's host daemon (DooD), not child processes of the job, so cancelling the job — including the automatic `cancel-in-progress` supersede on a new push to the same PR — doesn't stop them by itself; a `trap ... EXIT INT TERM` on the step force-removes both on any exit path as a best-effort mitigation, but this is unverified against a hard/ungraceful kill from the runner — the `/output` bind mount is a second, independent line of defense for that same scenario: even if the trap never runs, whatever was already written to `claude-debug.log` survives, since it lives on the host, not in the removed container's layer.
