...
**Environment**: an isolated, egress-restricted container (full clone, base history available) — full tool access (Read/Glob/Grep/Bash/Edit/Write, unrestricted `git`), so use whatever's natural, including `git -C`. This is still a static review, though: there's no build toolchain or test runner for this repo's stack in the sandbox, so ground findings in what you can read/diff, don't assume you can compile or execute the reviewed code. Any file you write here is discarded with the container — your only real output is the JSON object in your final message (step 4).

**OUTPUT RULE**: Response is machine-parsed. Think, run `/code-review` and `/security-review`, analyze freely — but your **final** message must be nothing but the step-4 object: either the bare JSON object, or that object inside exactly one ```` ```json ```` block with nothing after it. Both are accepted (the CLI runs with `--json-schema`, which prefers the bare object; the fenced form is the fallback when a skill invocation leaves the turn as plain text) — what matters is that the last thing in the message is that object and nothing else. Decide the full content before writing it, never leave it half-done to revise later. It must be syntactically valid JSON — no trailing commas after the last item in an array/object, all strings properly escaped — malformed JSON discards the entire review, so double-check before emitting it.
...
- If `pr-files.md` exists instead, the diff was too large for line-level review: its `## Production files` section lists every changed production file (never dropped for churn, unlike `## Test/generated files` which is capped) — the only exception is a PR pathological enough to exceed 500 of them, and in that case the heading itself says `TRUNCATED`, so read the heading before trusting the list as complete. Trust it and `Read` every one of them in full, the same as you would in a normal-size review. The budget (`--max-budget-usd`, `timeout-minutes`) is generous enough for dozens of files, and `pr-files.md` listing a file is not the same as reviewing it. You do not need raw `Bash` to reconstruct or double-check this list against `pr.diff` yourself — it is already complete; only sample/grep-by-impact within `## Test/generated files`, or if the production list itself is too large to fully read. `coverage_note` must reflect only the files you actually `Read` this session (a count against the total, e.g. "38 of ~40 production files read"), not the full set `pr-files.md` names.
...
- `<previous_review>` (if appended) lists prior open findings, numbered — used in step 2's incremental review. `<delta_diff>` (if appended - independent of `<previous_review>`, which is only appended when there were open findings last round) is just what changed since the last reviewed commit; it is reasoning context only, and does not decide any scope — step 2's `/code-review` range comes pre-resolved as `$REVIEW_RANGE`.
...
### 2. Build the review
Run `/code-review` and `/security-review` **every session, on every diff size — never skip either just because the diff (or the delta) looks small.** Neither skill reads this prompt's `<pr_diff>`/`<delta_diff>` blocks — each determines its own scope from git independently. `/code-review` supports an explicit range as its skill argument, never bare — the range is already resolved for you, so run exactly `/code-review $REVIEW_RANGE` (it is `$PREVIOUS_SHA...HEAD` on an incremental review and `origin/$BASE_BRANCH...HEAD` on a fresh one; never assemble it yourself). `/security-review` has no such argument — it's a built-in skill with a hardcoded `origin/HEAD...` scope (always the full PR, never just the delta); invoke it bare, `/security-review` with no argument. This is why the checkout step forces `origin/HEAD` to track `$BASE_BRANCH` (a plain clone's default otherwise points at the repo's overall default branch, not this PR's own base).

Run `/code-review` first (inherits this session's `--effort`, no level needed) — it covers general methodology (data flow, boundaries, error handling, backward-compat, concurrency, dangling references, performance, docs), so don't repeat that yourself. It has no host to report through here, so it answers in plain text as `{file, line, summary, failure_scenario}`; translate each finding you keep:
...
### 4. Output
End your final message with a single JSON object of this shape and nothing after it — bare, or in one ```` ```json ```` fenced block (see OUTPUT RULE above; both are parsed):
...
- Valid, self-contained JSON: no comments, no trailing commas, nothing after the object (or its closing fence).
