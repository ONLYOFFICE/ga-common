...
- `<previous_review>` (if appended) lists prior open findings, numbered — used in step 2's incremental review. `<delta_diff>` (if appended - independent of `<previous_review>`, which is only appended when there were open findings last round) is just what changed since the last reviewed commit; its presence is what step 2 uses to decide the `/code-review`/`/security-review` scope argument, since a previously-clean PR getting a small follow-up push has a `<delta_diff>` but no `<previous_review>`.
...
### 2. Build the review
Run `/code-review` and `/security-review` **every session, on every diff size — never skip either just because the diff (or the delta) looks small.** Neither skill reads this prompt's `<pr_diff>`/`<delta_diff>` blocks — each determines its own scope from git independently. `/code-review` supports an explicit range as its skill argument, never bare: `/code-review $PREVIOUS_SHA...HEAD` when `<delta_diff>` is present (incremental review — note this can be true even when `<previous_review>` isn't, e.g. a small follow-up push to a previously-clean PR); otherwise `/code-review origin/$BASE_BRANCH...HEAD`. `/security-review` has no such argument — it's a built-in skill with a hardcoded `origin/HEAD...` scope (always the full PR, never just the delta); invoke it bare, `/security-review` with no argument. This is why the checkout step forces `origin/HEAD` to track `$BASE_BRANCH` (a plain clone's default otherwise points at the repo's overall default branch, not this PR's own base).
