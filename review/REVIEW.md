...
### 2. Build the review
Run `/code-review` and `/security-review` **every session, on every diff size — never skip either just because the diff (or the delta) looks small.** Neither skill reads this prompt's `<pr_diff>`/`<delta_diff>` blocks — each determines its own scope from git independently, so invoke both with an explicit range as the skill argument, never bare: `/code-review $PREVIOUS_SHA...HEAD` (and the same argument for `/security-review`) when `<delta_diff>` is present (incremental review — note this can be true even when `<previous_review>` isn't, e.g. a small follow-up push to a previously-clean PR); otherwise `/code-review origin/$BASE_BRANCH...HEAD`.
