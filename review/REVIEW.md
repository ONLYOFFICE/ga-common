...
- For a long production-file list, you may dispatch subagents (Task tool) to read/analyze files in parallel and report findings back — you're in the same sandbox either way, this is purely a wall-clock optimization; still ground `coverage_note` in what was actually read, subagent reads included.
- Use `Glob`/`Grep`/`Read` for callers/callees of changed code, not just the changed lines themselves — for a wide blast radius (many call sites), dispatch one subagent per site in parallel rather than walking them one by one.
- `pr.diff` is base→head only, so a later commit reverting/weakening an earlier one is invisible in it. For multi-commit PRs, `git log --oneline origin/$BASE_BRANCH..HEAD` then `git show <sha>`/`git diff <sha>^..<sha>` on suspicious ones (wip/fix/revert, or touching already-changed security-relevant lines) — dispatch one subagent per suspicious commit in parallel when there are several.
